...
from src.data_loading import create_dataset_summary, download_dataset, load_activity_data
...
from src.preprocessing import preprocess_dataset, preprocess_signal
...
def run_task1_dataset_exploration(output_dir):
...
def run_task2_annotated_signals(data_list, output_dir):
...
def run_task3_preprocessing(signal_data, output_dir):
...
def run_task4_windowing(preprocessed_data, output_dir):
...
def run_task5_feature_extraction(preprocessed_data, output_dir):
...
def run_task6_modeling(features_df, feature_cols, output_dir):
    ...
    minority_weight_overrides = {
        'sitting': 1.10,
        'walking': 1.35,
    }
    
    results_df, trained_models = train_classical_models(
        X_train,
        y_train,
        X_test,
        y_test,
        class_names=le.classes_,
        class_weight_overrides=minority_weight_overrides,
    )
    ...
    return best_model, le, X_train, y_train, X_test, y_test


def run_task7_evaluation(features_df, feature_cols, best_model, le, X_train, y_train, 
...
def main():
    ...
    log_filename = output_dir / f"console_output_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
    ...
    try:
        ...
        print("="*60)
        print(f"\nConsole output is being logged to: {log_filename}")
        print("="*60)
        
        data_list, metadata = run_task1_dataset_exploration(output_dir)
        signal_data = run_task2_annotated_signals(data_list, output_dir)
        preprocessed_data = run_task3_preprocessing(signal_data, output_dir)
        run_task4_windowing(preprocessed_data, output_dir)
        features_df, feature_cols = run_task5_feature_extraction(preprocessed_data, output_dir)
        best_model, le, X_train, y_train, X_test, y_test = run_task6_modeling(
        ...
        print(f"  - {log_filename.name} (console output log)")
        print("="*60)
    
    finally:
        tee.close()
        print(f"\nConsole output log saved to: {log_filename}")
